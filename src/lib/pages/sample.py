...
class SamplePage(ctk.CTkScrollableFrame, BaseComponent):
    ...
    def __init__(self, master: ctk.CTk, event_bus: EventBus, **kwargs) -> None:
        ...
        self.sample_items[W.LABEL].grid(row=2, column=0, pady=10)
        # CTkEntry
        self.sample_items[W.ENTRY] = ctk.CTkEntry(master=self)
        self.sample_items[W.ENTRY].grid(row=2, column=1, pady=10)
        # CTkButton
        self.sample_items[W.BUTTON] = ctk.CTkButton(master=self)
        self.sample_items[W.BUTTON].grid(row=3, column=0, pady=10)
        # CTkSegmentedButton
        self.sample_items[W.SEGMENTEDBUTTON] = ctk.CTkSegmentedButton(
            master=self,
            values=['button1', 'button2', 'button3'],
        )
        self.sample_items[W.SEGMENTEDBUTTON].grid(row=3, column=1, pady=10)
        # CTkCheckBox
        self.sample_items[W.CHECKBOX] = ctk.CTkCheckBox(master=self)
        self.sample_items[W.CHECKBOX].grid(row=4, column=0, pady=10)
        # CTkRadioButton
        self.sample_items[W.RADIOBUTTON] = ctk.CTkRadioButton(master=self)
        self.sample_items[W.RADIOBUTTON].grid(row=4, column=1, pady=10)
        # CTkSwitch
        self.sample_items[W.SWITCH] = ctk.CTkSwitch(master=self)
        self.sample_items[W.SWITCH].grid(row=5, column=0, pady=10)
        # CTkProgressBar
        self.sample_items[W.PROGRESSBAR] = ctk.CTkProgressBar(master=self)
        self.sample_items[W.PROGRESSBAR].grid(row=6, column=0, pady=10)
        # CTkSlider
        self.sample_items[W.SLIDER] = ctk.CTkSlider(master=self)
        self.sample_items[W.SLIDER].grid(row=6, column=1, pady=10)
        # CTkOptionMenu
        self.sample_items[W.OPTIONMENU] = ctk.CTkOptionMenu(
            master=self,
            values=['option1', 'option2', 'option3'],
        )
        self.sample_items[W.OPTIONMENU].grid(row=7, column=0, pady=10)
        # CTkComboBox
        self.sample_items[W.COMBOBOX] = ctk.CTkComboBox(
            master=self,
            values=['combo1', 'combo2', 'combo3'],
        )
        self.sample_items[W.COMBOBOX].grid(row=7, column=1, pady=10)
        # CTkTextbox
        self.sample_items[W.TEXTBOX] = ctk.CTkTextbox(master=self)
        self.sample_items[W.TEXTBOX].grid(row=8, column=0, columnspan=2, pady=10)
        # DropdownMenu
        # [Attetion]
        ...
        # CTkScrollableFrame-Label
        # [Attention]
        # Set this to check the label_fg_color of CTkScrollableFrame.
        self.configure(label_text='Sample (CTkScrollableFrame Label)')
